...
SCX (Sparse Cell eXpression System) is a purpose-built binary file format, compression codec, query engine, and ML data loader for single-cell RNA-seq data. It replaces AnnData/h5ad with a unified Rust-native stack — 3-7× smaller files, fastest reads at census scale, 4-44× less memory, a GPU-saturating training loader, lazy query engine, and Rust-native analysis accelerators. Native Python (`pyscx`) and R (`rscx`) bindings; compatible with scverse and Seurat v5.
...
14 workspace crates; the dependency core is `scx-codec → scx-format → {scx-mtx, scx-ops, scx-engine, scx-loader, scx-cloud, scx-gpu, scx-accel} → {scx-cli, pyscx, rscx}`. Full graph and isolation rules in [docs/architecture.md § Crate Dependency Graph](docs/architecture.md#crate-dependency-graph). Feature flags: `scx-cli{hdf5,cloud}`, `pyscx{cloud,gpu}`, `scx-gpu{gds}`, `scx-accel{gpu}` — all opt-in.

File format ([docs/format.md](docs/format.md)): 256-byte LE header (magic `b"SCX\x01"`), 4096-byte root catalog at offset 256, 8-byte-aligned sections (15 types), full catalog at EOF with per-entry checksums + shard statistics. 76-byte CSR shard headers (magic `b"SCXS"`). Codecs: `None`, `Scx1` (Delta-Golomb + FOR-BP + Rice, integer only), `Zstd`, `Lz4Shuffle`; auto-codec selects Scx1 vs Zstd by median value. On-disk `u64`/`u16-u32`/`u8-u32`, in-memory `i64`/`i32`/`f32` to match scipy CSR zero-copy.
...
- **Lazy preprocessing**: `ScxLazyTransformedDataset` chains `normalize_total → log1p → row_scale` without materialization; `ShardSource` enables streaming PCA. Full out-of-core pipeline (open → QC → normalize → log1p → HVG → PCA → kNN → UMAP → Leiden) hits **5.1 GB peak RSS on 1M cells** (88% reduction vs 43.6 GB baseline).
...
- **GPU pipeline speedup**: 6.9× median on 1M cells (10× target not yet met). Best individual: UMAP 18.8× / kNN 5.4× in-pipeline. PCA at 2K HVG × 1M is 1.0× (CPU covariance PCA already ~3 s). Post-dispatch-reframe Leiden reaches cuGraph directly via `device="gpu"` (~16× on 1M cells). Routing details: [benchmarks/README.md § GPU accelerator regression workflow](benchmarks/README.md#gpu-accelerator-regression-workflow).
- **GPU Leiden correctness**: ARI ≈ 0.92 vs Python `leidenalg` — documented behaviour of `device="gpu"` / `"auto"` on GPU hosts, not a regression. Pin `device="cpu"` to preserve label stability for downstream DE / annotation transfer.
- **Benchmark baseline scope**: capture and gate coverage differ. `LATEST` → `v0.6.2-n_counts-augmentation` (full-surface, 806 rows; 686/806 with `wall_s_iqr` for variance-aware per-row tolerance). Prior baselines (`v0.6.0-gpu-phase1-7-multidataset`, `v0.6.0-gpu-phase1-7`, `v0.6.1-multimodal`) remain under `results/baselines/` for historical A/B. Justifications now suppress both regression and absolute-floor violations on a triple (fixed 2026-05-11). Deferred floors are catalogued in `benchmarks/comprehensive/thresholds.yaml` § "Deferred floors": SLAF `ml_loader` (separate conda env), `avg_gpu_util_pct__gpu_train` (needs in-process sampler), `census_1m × cell_eval_parity_perf` (cell-eval reference is O(N²)).
...
- **Multimodal limitations**: `scx merge` / `scx compact` not yet implemented (extract single modalities first via `scx subset --modality NAME`); per-modality CSC sidecar preservation on `scx append` is a follow-on. Spatial transcriptomics with R-tree spatial index is its own follow-on spec.
