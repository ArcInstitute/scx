...
- **Analysis accelerators** (`scx-accel`, [docs/scanpy.md § Rust-native accelerators](docs/scanpy.md#rust-native-accelerators)): PCA (covariance / randomized auto-routed), kNN (HNSW), UMAP, differential expression (pre-ranking Wilcoxon), Leiden (Rust-native CPU + cuGraph GPU), Harmony2 batch integration, LISI, HVG (streaming seurat_v3 / seurat), pseudobulk, perturbation evaluation metrics (cell-eval / arc-bench parity). An **accelerator execution planner** (`scx_accel::route::plan_de_route`) decides and records the DE route + fallback reason on `adata.uns["scx_accel"][<op>]` (`route` ∈ `cpu_*`/`gpu_csr_v{1,2,3}`/`gpu_csc_v3`/…); route-specific benchmark gates (`de_route_csc_direct`, `wilcoxon_route_gpu_correct`, `csc_dispatch_correct`, `hvg_route_gpu_correct`, `pca_route_gpu_correct`, `knn_route_gpu_correct`, `umap_route_gpu_correct`, `leiden_route_gpu_correct`) in `thresholds.yaml` turn silent GPU→CPU or CSC→CSR dispatch fallbacks into hard gate failures across all GPU accelerator ops. See [docs/api.md § Accelerator route metadata](docs/api.md#accelerator-route-metadata).
- **GPU acceleration** (`scx-gpu`, [docs/gpu-setup.md](docs/gpu-setup.md)): `device="auto"|"cpu"|"gpu"|"gpu:N"` selector across `pyscx.accel.*`. GPU PCA (cuSPARSE + cuBLAS, covariance ≤ 8000 vars / randomized > 8000; the CPU PCA accelerator uses a 5000-var cutoff), GPU kNN (cuVS CAGRA), GPU UMAP, GPU Leiden (cuGraph), GPU preprocessing (`normalize_total`/`log1p`/`highly_variable_genes`). Double-buffered shard I/O; graceful CPU fallback.
