...
- **Benchmark baseline scope**: capture and gate coverage are not the same. The capture phase runs every entry in `ALL_BENCHMARKS`; the gate phase only flags rows that exist in the chosen baseline. The current `LATEST` symlink points at `v0.6.0-gpu-phase1-7-multidataset` and is **accel-only** (60 rows: `accel_hvg / knn / leiden / pca / preprocess / umap`). `compression / read / write / parallel_scaling / memory / cloud / ml_loader / index_plan / correctness / roundtrip / cell_eval_parity_perf / fragment_ops` capture cleanly but produce no gate signal against `LATEST` — pin `--baseline benchmarks/comprehensive/results/baselines/v0.6.0-gpu-phase1-7` (577 rows, format + cloud + ml_loader + accel) for full-surface gating until a multi-surface baseline is promoted as `LATEST`. For one-off perf comparisons on benchmarks not in any baseline (e.g. `index_plan`), do an A/B run via the recipe in `benchmarks/README.md` § "Ad-hoc A/B for a single benchmark". Known gate-side gaps: SLAF floors for `ml_loader` (deferred — `slafpy` lives in a separate `scx-bench-slaf` conda env and the orchestrator activates one env per run), `avg_gpu_util_pct__gpu_train` floors (1 Hz `nvidia-smi dmon` misses sub-second epochs; needs an in-process sampler), and `census_1m` `cell_eval_parity_perf` coverage (energy_distance variants skip at `n_obs >= 500K` because the cell-eval reference's O(N²) work is infeasible at that scale).
- **CSC storage**: Optional gene-major (`CscShard`) sidecar fully supported. Opt in at write time with `pyscx.from_anndata(csc="always")` / `scx convert --csc=always` / `scx build-csc`; multi-shard layouts via `--csc-cols-per-shard` (default 5000). Consumers select the column-major path with `prefer_format="csc"` on `pyscx.accel.{rank_genes_groups, highly_variable_genes, pseudobulk_dex, calculate_qc_metrics, col_sums, col_nnz, col_min, col_max, col_var}`; PCA explicitly rejects CSC (CSR is faster on the row-major hot paths). Mutating ops (`append`, `compact`, `merge`, `subset`) drop the sidecar by default with a warning; pass `--rebuild-csc` to re-emit. See [docs/sharding.md § CSC sharding](docs/sharding.md#csc-sharding) and [docs/scanpy.md § prefer_format](docs/scanpy.md#prefer_formatcsrcsc-explicit-column-major-dispatch).
- **Multimodal**: CITE-seq / 10x Multiome / TEA-seq supported via the v2 format (`pyscx.from_mudata` / `to_mudata`, `rscx::from_seurat` v5 multi-assay, `rscx::from_mae`, `pyscx.MultimodalTrainingDataset`). See [docs/multimodal.md](docs/multimodal.md). Multimodal `scx merge` and `scx compact` are not yet implemented (extract single modalities first via `scx subset --modality NAME`); per-modality CSC sidecar preservation on `scx append` is also a follow-on (the file-wide drop applies uniformly today). Spatial transcriptomics with an R-tree spatial index is its own follow-on spec.
