...
- **GPU pipeline speedup**: 6.9x median achieved on 1M cells post-Phase-7 (up from 3.8x pre-Phase-1; 10x target still not met). Best individual after Phase 1-7: UMAP 18.8x on census_1m (21.9x standalone), kNN 5.4x in-pipeline (4.8x standalone). Leiden at parity (Rust-native runs first per Phase 6; pre-baseline's 16x measured cuGraph directly). PCA at 2K HVGs × 1M is 1.0x — CPU covariance PCA already ~3s, no GPU headroom; at n_vars=100K PCA reaches 1.7x. Full regression report at `benchmarks/results/phases_1_7_gpu_regression_report.md` from Phase 8 cluster run (job 2211369, 2026-04-23). See `GPU-ACC-SPEED-UP.md` Phase 8 and [benchmarks/README.md § GPU accelerator regression workflow](benchmarks/README.md#gpu-accelerator-regression-workflow).
