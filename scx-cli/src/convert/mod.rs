...
pub mod mtx_pipeline {
    ...
    pub fn mtx_to_scx(
        ...
        let mtx_data = scx_mtx::read_mtx_directory(input_dir)?;

        let explicit_codec = match codec_str {
            "auto" => None,
    ...
    /// Detect value encoding and auto-select codec.
    ...
    /// Convert f32 data to raw LE bytes.
    ...
    /// Write CSR shards to the ScxWriter.
    #[allow(clippy::too_many_arguments)]
    fn write_csr_shards(
        writer: &mut ScxWriter,
        indptr: &[i64],
