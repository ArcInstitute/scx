...
- [x] `scx build-csc input.scx [output.scx]` — one-pass streaming transpose, appended to the file in place (rollback-able; copy-out with `output`). **Fully integrated** end-to-end: write-time CSC at `scx convert --csc=always` / `pyscx.from_anndata(csc="always")` / 10x / MTX import; multi-shard CSC (`--csc-cols-per-shard`, default 5000); column-major streaming via `BackedCscReader` + `ColumnShardSource` trait; consumer dispatch via `prefer_format="csc"` on HVG / DE / pseudobulk / QC / col_aggs; `scx info` per-shard layout; `scx validate` BLAKE3 coverage; rewrite ops (`compact` / `merge` / `optimize` / `sort` / `subset`) carry CSC by default, rebuilt in the same pass as X (`--csc carry|always|off`); `append` drops it (`--rebuild-csc` rebuilds in place); ingest defaults to `--csc auto` (a sidecar at ≥ 50,000 cells × 5,000 genes; `--csc off` opts out), with the builder parallel and in the same pass as X. See [docs/sharding.md § CSC sharding](docs/sharding.md#csc-sharding) and [docs/scanpy.md § prefer_format](docs/scanpy.md#prefer_formatautocsrcsc-column-major-dispatch).
