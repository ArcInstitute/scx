...
- Accel `device=` selector: `rscx` accelerators are CPU-only. (Route *metadata*
  is no longer a gap: since ORG-10.16-3b every rscx accelerator stamps the same
  `scx-accel` planner record pyscx writes to `uns["scx_accel"]` —
  `object@misc$scx_accel[[op]]` on Seurat input, a `scx_accel` list element /
  attribute on matrix input — gated by the `accel_r_route` benchmark floors.)
