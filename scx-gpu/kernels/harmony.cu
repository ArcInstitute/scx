...
//
// Produces `R[:, i] = softmax(-dist[:, i] / sigma)` for every cell —
// no diversity penalty term. Used by the GPU orchestrator at iter > 0
// cold-start, where the penalty is *not* applied yet (CPU
// `softmax_r_from_dist` does the same; the per-block penalty is folded
// into `update_r`'s inner softmax via leave-block-out O/E).
//
// One thread block per cell (up to blockDim.x = 256), cooperating on
// the softmax reduction via shared memory; for K > 256 threads loop
// with stride.
// ──────────────────────────────────────────────────────────────────────
...
//
// Softmax with the diversity penalty folded in, over an explicit list
// of cell indices `block_cells[blockIdx.x]` rather than all N cells.
// Each block in the grid handles one cell.
//
//   diversity[k, i] = prod_c ((2*E[k, b_c(i)] + 1)
//                             / (O[k, b_c(i)] + E[k, b_c(i)] + 1)) ^ theta[b_c(i)]
//
// then column-normalize so R[:, i] sums to 1. `batch_labels_flat`
// encodes per-covariate labels as a row-major (C x N) int32 matrix, so
// one cell's C batch levels are a stride-N lookup; `cov_offset[c]` is
// the global batch index of covariate c's level 0. The penalty uses the
// *current* O/E values — callers pass leave-block-out O/E by
// subtracting the block's contribution before launching.
// ──────────────────────────────────────────────────────────────────────
