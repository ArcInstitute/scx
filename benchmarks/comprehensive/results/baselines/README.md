...
**`LATEST` points at `v0.16.0-opt-instruments`** (since 2026-09-03). 1763 rows
across 51 benchmark families and 13 datasets, multi-surface, threads unpinned.
...
Rows for two retired third-party comparison formats have been
removed from every snapshot; the row counts below are after that removal, and
each snapshot's `MANIFEST.sha256` was updated to match.
...
| `v0.11.0-recapture` | 2026-07-11 | full + accel | 1451 rows from `baseline_2026_07_10`. |
| `v0.11.0-auto-converge` | 2026-07-12 | full + accel | 1450 rows from `candidate_auto_converge_20260712` — the `auto` → cost-aware-adaptive flip. `scx_auto_v2` rows disappear here by design. |
| `v0.11.2-multimodal-loader-fix` | 2026-07-12 | full + accel + multimodal | Same 1450-row snapshot as `v0.11.0-auto-converge`, re-promoted after the multimodal loader fix. `pyscx 0.11.0`. **Zero rows for 13 of the 35 floored benchmarks** — a gate over those tests absolute floors only and performs no regression comparison, which is what a recapture has to fix. |
| `v0.11.5-dataload-phase0` | 2026-07-23 | xl + accel, no multimodal | 1496 rows; first with `cellset_gather` / `obs_open` / `ooc_loader`. Deliberately **not** promoted — see "Picking a baseline". |
