#!/bin/bash
# Orchestrator sentinel: runs run_parallel.py for the xl-tier dataset list
# (small tier + census_500k + census_1m + census_5m + multimodal). Wraps the
# orchestrator in a long-walltime sbatch so it survives ssh drops and can be
# monitored with `squeue` + watch.py.
#SBATCH --job-name=scx-bench-xl
...
echo "=== xl orchestrator boot ==="
echo "host=$(hostname) job=${SLURM_JOB_ID:-none} time=$(date -Iseconds)"
echo "python=$(which python)"
python --version
echo "============================"

# xl tier datasets — TIERS["xl"] from capture_baseline.py plus the multimodal
# pair so multimodal_* benchmarks pick up coverage in the xl run too.
