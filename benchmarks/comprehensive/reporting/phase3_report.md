...
- [ ] **3G.4** Run the full Phase 3 benchmark suite on all datasets. This is the final exit benchmark:
  ...
  - **Status:** 36 parallel SLURM jobs submitted (jobs 1983892–1983927). Jobs still running/pending — results not yet assessed. Covers 6 benchmark types (compression, write, read_full, read_selective, parallel_scaling, memory) × 6 datasets (D1–D4 + census_500k + census_1m) across all SCX codec variants (scx_auto, scx_scx1, scx_zstd, scx_lz4, scx_pcodec, scx_none) and competing formats (h5ad_gzip, h5ad_lzf, zarr_zstd, zarr_lz4, tiledb_soma).
- [ ] **3G.5** Generate the final post-Sprint-3 benchmark report comparing all metrics against the pre-Sprint-1 baseline and against all competing formats. Quantify cumulative improvement across all three sprints.
  - **Status:** Report generator script created at `benchmarks/scripts/generate_phase3g_report.py`. Preliminary report at `benchmarks/results/phase3g_final_report.md` was generated from pre-existing results (486 current vs 382 baseline). Needs to be re-run after 3G.4 SLURM jobs complete to include the fresh benchmark data.
