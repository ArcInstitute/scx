...
- [x] **3G.4** Run the full Phase 3 benchmark suite on all datasets. This is the final exit benchmark:
  ...
  - **Result:** 36 parallel SLURM jobs (1983892–1983927) all COMPLETED with exit code 0:0. 506 result JSON files in `benchmarks/comprehensive/results/raw/`. Covers 6 benchmark types (compression, write, read_full, read_selective, parallel_scaling, memory) × 6 datasets (D1–D4 + census_500k + census_1m) across all SCX codec variants (scx_auto, scx_scx1, scx_zstd, scx_lz4, scx_pcodec, scx_none) and competing formats (h5ad_gzip, h5ad_lzf, zarr_zstd, zarr_lz4, tiledb_soma).
- [x] **3G.5** Generate the final post-Sprint-3 benchmark report comparing all metrics against the pre-Sprint-1 baseline and against all competing formats. Quantify cumulative improvement across all three sprints.
  - **Result:** Report regenerated at `benchmarks/results/phase3g_final_report.md` after all 3G.4 jobs completed. Compared 506 current results against 382 pre-Sprint-1 baseline results. Key findings:
    - **Median read/write improvement: 81.9%** across all SCX formats and datasets (Sprint 1+2+3 cumulative)
    - **Best improvement: 96.8%** (scx_scx1 write on census_500k: 481.5s → 15.6s)
    - **Worst case: +33.4%** (scx_auto read_full on pbmc3k — 0.031s → 0.041s, within noise on a tiny dataset)
    - Read full: 0.1–91.7% faster; census_1m 87.5% faster (22.9s → 2.9s), tabula_sapiens_100k 59.3% faster (1.5s → 0.6s)
    - Read selective: 84–94% faster; census_1m 94.3% faster (306s → 17.4s)
    - Write: 17–97% faster; census_1m scx_scx1 96.7% faster (898s → 30.0s)
    - SCX (auto) vs h5ad_gzip on census_1m: 17× faster reads (48.5s → 2.9s), 5.2× faster writes (232s → 44.6s)
    - SCX (auto) vs tiledb_soma on census_1m: 6.5× faster reads (18.7s → 2.9s), 7.0× faster writes (312s → 44.6s)
    - Sprint 3 new codecs: LZ4Shuffle fastest writes (27.5s census_1m); Pcodec matches Zstd compression with slightly faster reads on small datasets; both provide full large-dataset coverage (census_500k, census_1m)
