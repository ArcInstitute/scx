...
variants declare the ``cloud_read`` capability (via ``scx_runner``) but have
no cloud layout, so ``DatasetConfig.cloud_url`` raises ``ValueError`` for
them — pinning the allow-list keeps them out of the cohort (a failed cloud
job otherwise breaks the next cohort's ``afterok:`` dependency). Read by
``run_parallel.py``'s cohort builder; mirrored by the runtime guard in
``run()``."""
