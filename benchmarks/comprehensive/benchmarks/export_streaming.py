...
def run(
    ...
    # Refuse a sweep above the materialize cap BEFORE resolving or converting any
    # input. The cap has to guard this branch as well as `_run_isolated`'s:
    # `_run_thread_count_subprocess` unconditionally appends the materialize arm,
    # so a sweep above the cap still loaded the whole CSR triplet (~70 GB at
    # census_5m, ~140 GB at census_10m) — exactly what the cap prevents. Found by
    # all three round-2 reviewers on PR #451.
    #
    # Refused rather than silently skipped, mirroring `conversion_streaming.run`:
    # a sweep exists to compare how the two arms scale, so dropping one makes its
    # output meaningless, and running a ~140 GB arm the operator did not know they
    # asked for is worse.
    #
    # Placed here, ahead of input resolution, so the refusal costs nothing and is
    # reachable from a unit test — round 1's cap shipped with a test that asserted
    # the constants and never drove the branch, which is how the bypass survived.
    if thread_counts is not None and _skip_materialize_reason(dataset.n_obs):
        raise ValueError(
            ...
            f"compares the two arms, so it needs both: either use a dataset at or "
            f"below n_obs={MATERIALIZE_MAX_N_OBS:,}, or unset "
