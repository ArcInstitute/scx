...
pub fn read_dataframe_group(
    ...
    // `scx-convert/src/h5ad/column_stream.rs::write_dataframe_header`) rely on
    ...
    // `scx-convert/src/h5ad/column_stream.rs::write_dataframe_header`'s
...
fn uns_dataframe_column_envelope(ds: &hdf5::Dataset) -> Result<serde_json::Value, ConvertError> {
    ...
    match desc {
        ...
        | TypeDescriptor::FixedUnicode(_)
        | TypeDescriptor::FixedAscii(_) => {
            let raw: Vec<hdf5::types::VarLenUnicode> = ds.read_1d()?.to_vec();
...
fn read_uns_dataframe_group(
    ...
    let index_ds = group.dataset(&index_name)?;
...
fn read_uns_entry(
    ...
    if let Ok(subgroup) = group.group(name) {
        ...
        if enc == "dataframe" {
            ...
            return read_uns_dataframe_group(&subgroup, name, sink);
