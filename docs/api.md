...
- `pyscx.attach_obs_columns(path, df, *, key=None, positional=False, status_column=None, uns=None, uns_key=None, overwrite=False, on_missing_rows="null", on_extra_rows="warn", dry_run=False)` —
  ...
  written as handed in); any other length raises naming both counts. Under a
  live-length attach `n_matched` is the live count and `n_target_rows_absent`
  the deleted rows; provenance records `row_space`. `status_column` is
