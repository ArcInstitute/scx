...
`optimize_with_framing(input, output, codec, obs_shard_policy, framing, memory_budget) → Result<OptimizeStats>` is the full form; `optimize` delegates to it with no framing and no budget.
