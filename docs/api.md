...
- `pyscx.accel.adjusted_rand_index(labels_a, labels_b, rescaled=False) → float` — ARI. Default matches `sklearn.metrics.adjusted_rand_score` (range `[-0.5, 1]`, consistent with NMI/AMI above). Pass `rescaled=True` for cell-eval's `(ARI + 1) / 2` in `[0, 1]`.
