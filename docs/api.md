...
- `pyscx.from_mtx(mtx_dir, scx_path, codec=None, shard_size=None, shard_obs="auto", allow_lossy=False, csc=None, csc_cols_per_shard=5000)` — `csc=None` is `"auto"`, as on the other ingest entry points; the sidecar is appended in place after the MTX read, which writes CSR only. Cell Ranger MTX directory (`matrix.mtx[.gz]`, `barcodes.tsv[.gz]`, `features.tsv[.gz]`) to SCX. Default shard size is 16384.
