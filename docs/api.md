...
| `rank_genes_groups` (Wilcoxon rank-sum) | ✅ | **`auto`** | ✅ (`gpu_csc_v3`) | CSC-direct GPU route by default when a CSC sidecar is present — including on a windowed or transformed handle, which used to be downgraded to `gpu_csr_v3`; in-memory CSR still falls back to `gpu_csr_v3`. On CPU the `auto` default takes the CSC kernel whenever the file has a sidecar: neither a lazy transform chain, nor a row filter, nor a column projection disqualifies, so an ordinary `filter_cells → filter_genes → normalize_total → log1p` pipeline now routes CSC. |
