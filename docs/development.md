...
The job skips scx-convert's 7 ungated module prefixes and selects scx-cli's 5
hdf5-gated test binaries (`convert_stream`, `cellbender_cli`, `convert_subset`,
`doublet_import_hdf5`, `var_import_hdf5`); a dedup-guard step pins both selection lists against
...
serialization exists for the four binaries that reference the `hdf5` crate
