...
| `SCX_ACCEL_WILCOXON_NNZ` | `scx-accel` | unset | `1`/`true`/`TRUE`/`on` opts the **CPU CSC** Wilcoxon kernel into the exact sparse-nnz path (§5.3): each gene's nonzeros plus an analytic implicit-zero tie block, instead of a dense `n_obs`-length per-gene sort. Numerically equivalent (property-tested to ~1e-9). 1-vs-rest only — an explicit `reference=` or `rankby_abs=True` keeps the densify path whatever this is set to. When it is taken the route records as **`cpu_csc_nnz`** rather than `cpu_csc`, so a benchmark can tell which kernel it timed. Read once, through a `OnceLock`, so it must be set before the process's first CSC DE call. |
