...
| `cpu_profile_reduction_ms` / `_count` | Per-shard kernel accumulation (disjoint from decode; includes the DE ranking pass). |
...
`pyscx.accel.cpu_profile_snapshot()` / `cpu_profile_reset()`. **Known gaps**
(not captured in the `decode` bucket): the CSC-sidecar decode path (a separate
route from the row-major CSR reader that carries the central hook), and the cloud
range-read path (local mmap only in the current captures).
