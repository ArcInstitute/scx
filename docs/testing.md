...
| **QC / filtering** | `test_calculate_qc_metrics_empty_qc_var.py`, `test_calculate_qc_metrics_mt_warning.py`, `test_qc_metrics_projection.py`, `test_qc_metrics_fused.py` | QC edge cases (empty qc_var, MT warning); column-projection correctness on both axes and both routes; fused-vs-unfused bit-identity and a `cpu_profile_snapshot()` pass-count pin |
