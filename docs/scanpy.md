...
> differential expression (`pdex_ref`, `rank_genes_groups`) the column-major
> CSC-direct GPU route is the high-performance path, and it requires a *backed*
> SCX file with a CSC sidecar (`pyscx.from_anndata(..., csc="always")` /
> `scx convert --csc=always`) plus `SCX_GPU_DE_V3=1`; in-memory scipy CSR can
> run on GPU but may be slower than CPU. When comparing performance, **check
> the recorded route** at `adata.uns["scx_accel"][<op>]["route"]` (e.g.
> `gpu_csc_v3` vs `gpu_csr_v3`) — every DE call records which route it actually
> took and the `fallback_reason` if it didn't take the ideal one. See
