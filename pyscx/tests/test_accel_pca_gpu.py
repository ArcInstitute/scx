...
#   `device="gpu"` PCA routes to rapids-singlecell, and the native GPU path
#   dispatches randomized unconditionally), so there is no GPU covariance path to compare
...
def test_normalize_log1p_pca_matches_scanpy(tmp_path):
    ...
    * It is not a *GPU-route* gate. Nothing here asserts
    ...
    # and that wording was copied into docs/scanpy/accel-gpu.md's tolerance
    # table as a route claim it does not support.
