...
def to_h5ad(path, out, **kwargs):
    ...
    Args:
        ...
        memory_budget: Derates reader_threads against the exact per-shard
            ...
            The budget is evaluated per matrix written — X, each layer, and
            (since /raw began streaming) adata.raw. Size it for the LARGEST
            of those, not for X: raw is captured before HVG subsetting, so
            its shards are often the widest and are usually the binding
            constraint on a .raw-bearing file. Because raw is written last,
            a budget that admits X but not raw raises only after /X and the
            layers are already on disk, leaving a partial output file.
